...
如果未定义析构函数，`HasPtr` 对象被销毁时将无法释放其成员 `ps` 所指向的动态内存，造成内存泄漏。
...
- 拷贝构造函数分配新的动态内存用来保存被拷贝成员值的拷贝，并将指向这块内存的 `shared_ptr` 保存在正在创建的对象中
- 拷贝赋值运算符分配新的动态内存用来保存被拷贝成员值的拷贝，并将指向这块内存的 `shared_ptr` 拷贝给被赋值对象的 `shared_ptr` 成员（递减原来指向对象的引用计数，递增新指向对象的引用计数，正确处理自赋值）
- 当 `StrBlob` 对象被销毁时，其 `shared_ptr` 成员也被销毁，会递减其指向的对象的引用计数，当指向一个对象的最后一个 `shared_ptr` 被销毁时，会自动销毁此对象，因此使用合成析构函数就可以满足要求
...
对 `swap` 的调用根据实参类型进行匹配：`swap(lhs.ps, rhs.ps);` 调用的是 `swap(std::string*, std::string*)`；而 `swap(lhs.i, rhs.i);` 调用的是 `swap(int, int)`，所以不会导致递归循环。

### [练习 13.30](13.30.hpp)

> 为你的类值版本的 HasPtr 编写 swap 函数，并测试它。为你的 swap 函数添加一个打印语句，指出函数什么时候执行。

使用重排元素顺序的算法交换两个元素时会调用 `swap`。
...
不会受益，类指针的 `HasPtr` 进行交换时，其拷贝构造函数和拷贝赋值运算符都不需要分配新的 `string`，只需进行指针的交换。
...
成员 `save` 和 `remove` 向一个给定的 `Floder` 添加一条 `Message` 或是从中删除一条 `Message`，会修改参数的内容，因此不能使用传值参数或者 `const` 的参数。
...
使用合成的拷贝控制成员，拷贝一个 `Message` 或将一个 `Message` 对象赋予另一个 `Message` 对象时，将无法更新 `Folder` 集合。
...
> 为 Message 类添加成员，实现向 folders 添加或删除一个给定的 Folder*。这两个成员类似 Folder 类的 addMsg 和 remMsg 操作。

### 练习 13.38

> 我们并未使用拷贝和交换方式来设计 Message 的赋值运算符。你认为其原因是什么？

使用拷贝和交换方式（第 459 页），参数传递时会调用拷贝构造函数拷贝右侧对象，将右侧对象的副本加入其所在的 `Folder` 中；在赋值运算符结束时，会调用析构函数销毁右侧对象的副本，将其从所在的 `Folder` 中删除。当发生自赋值时，这种方式会将 `Message` 从它所在的 `Folder` 中删除，造成错误。
...
`first_free` 指向数组第一个空闲元素，此元素已经分配内存，但是未构造。`push_back` 将新元素添加到第一个空闲位置，再递增 `first_free`。如果使用前置递增运算符，则先递增 `first_free`，再将新元素添加到 `first_free` 所指向的位置，会导致数组中存在已分配内存，但没有构造的元素。
...
更倾向于使用 `for_each` 和 `lambda`，形式更简洁。
...
- 左值引用：不能绑定到要求转换的表达式、字面值常量或是返回右值的表达式；有持久的状态
- 右值引用：可以绑定到要求转换的表达式、字面值常量或是返回右值的表达式；只能绑定到临时对象，可以自由地接管所引用的对象的资源
...
- `int&& r1 = f();`。调用一个返回引用的函数得到左值，其他返回类型得到右值（第 202 页），因此 `f()` 返回右值，`r1` 使用右值引用
- `int& r2 = vi[0];`。下标运算符返回该位置上元素的引用（第 91 页）
- `int& r3 = r1;`。右值引用只能绑定到临时对象（第 471 页）
- `int&& r4 = vi[0] * f();`。右值引用可以绑定到要求转换的表达式（第 471 页）

### [练习 13.47](13.47.hpp)

> 对你在[练习 13.44](#练习-1344)（13.5 节，第 470 页）中定义的 String 类，为它的拷贝构造函数和拷贝赋值运算符添加一条语句，在每次函数执行时打印一条信息。

### [练习 13.48](13.48.cpp)

> 定义一个 vector\<String> 并在其上多次调用 push_back。运行你的程序，并观察 String 被拷贝了多少次。

调用 4 次 `push_back` 共拷贝 `String` 7次。
...
- [StrVec](13.49_StrVec.hpp)
- [String](13.49_String.hpp)
- [Message](13.49_Message.hpp)

### [练习 13.50](13.50.cpp)

> 在你的 String 类的移动操作中添加打印语句，并重新运行 13.6.1 节（第 473 页）的[练习 13.48](#练习-1348) 中的程序，它使用了一个 vector\<String>，观察什么时候会避免拷贝。

调用 `push_back` 以及 `vector` 重新分配内存时会使用移动构造函数来避免拷贝。

### 练习 13.51

> 虽然 unique_ptr 不能拷贝，但我们在 12.1.5 节（第 418 页）中编写了一个 clone 函数，它以值方式返回一个 unique_ptr。解释为什么函数是合法的，以及为什么它能正确工作。

函数返回的 `unique_ptr` 通过移动操作将返回值赋予对象，即“窃取”了将要被销毁的 `unique_ptr` 的数据，保证某个时刻只有一个 `unique_str` 指向一个给定对象。

### 练习 13.52

> 详细解释第 478 页中的 HasPtr 对象的赋值发生了什么？特别是，一步一步描述 hp、hp2 以及 HasPtr 的赋值运算符中的参数 rhs 的值发生了什么变化。

- `hp = hp2` 使用拷贝赋值运算符，先释放 `hp` 当前的 `string`，再从 `hp2` 对象拷贝 `string`，`hp2` 的值不发生变化
- `hp = std::move(hp2)` 使用移动赋值运算符，`std::move` 返回 `hp2` 的右值引用，赋值运算交换 `hp` 和 `hp2` 的值

### [练习 13.53](13.53.hpp)

> 从底层效率的角度看，HasPtr 的赋值运算符并不理想，解释为什么？为 HasPtr 实现一个拷贝赋值运算符和一个移动赋值运算符，并比较你的新的移动赋值运算符中执行的操作和拷贝并交换版本中的执行的操作。

因为 `HasPtr` 的赋值运算使用了拷贝并交换操作，造成不必要的内存占用。
...
新的移动赋值运算符只需拷贝右侧对象的值，不用进行交换。

### [练习 13.54](13.54.cpp)

> 如果我们为 HasPtr 定义了移动赋值运算符，但未改变拷贝并交换运算符，会发生什么？编写代码验证你的答案。

使用赋值运算符，编译时会报错 `error: use of overloaded operator '=' is ambiguous (with operand types 'HasPtr' and 'HasPtr')`。
...
表达式 `Foo(*this)` 是一个右值（临时对象），因此会调用 `Foo::sorted() &&`
...
上一章：[第 12 章 动态内存](../第%2012%20章%20动态内存)

下一章：[第 14 章 重载运算与类型转换](../第%2014%20章%20重载运算与类型转换)
