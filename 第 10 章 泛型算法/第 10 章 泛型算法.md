...
> 重写 [1.6 节](#../第%201%20章%20开始/第%201%20章%20开始.md#16-书店程序)（第 21 页）中的书店程序，使用一个 vector 保存交易记录，使用不同算法完成处理。使用 sort 和 [10.3.1 节](#练习-10121012cpp)（第 345 页）中的 compareIsbn 函数来排序交易记录，然后使用 find 和 accumulate 求和。
...
> 使用 list 代替 vector 重新实现 [10.2.3 节](#10.2.3-重排容器元素的算法)（第 343 页）中的去除重复单词的程序。
